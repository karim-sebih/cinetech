...
// const API_BASE_URL = 'https://api.themoviedb.org/3/discover/tv?include_adult=false&include_null_first_air_dates=false&language=en-US&page=1&sort_by=popularity.desc';
// const IMAGE_BASE_URL = 'https://api.themoviedb.org/3/tv/series_id/images';
...
async function fetchTvSeries(page = 1) {
    try {
        ...
        if (!response.ok) { // Fix this condition (removed the 'response.ok' check)
    ...
    function showtvSeriesDetails(tvSeriesId) {
        window.location.href = `detailseries.html?id=${tvSeriesId}`
    }
...
document.addEventListener('DOMContentLoaded', () => {
    fetchTvSeries(1); // Changed from fetchMovies to fetchTvSeries
});