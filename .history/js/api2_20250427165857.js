...
export async function fetchMovies(page = 1, genreId = null) {
  try {
    let url = `${BASE_URL}/movie/now_playing?language=fr-FR&page=${page}`;
    if (genreId) {
    ...
    console.log('Données brutes des films (fetchMovies) - Premiers éléments :', data.results.slice(0, 3)); // Afficher les 3 premiers éléments
    ...
    console.log('Films transformés (fetchMovies) - Premiers éléments :', transformedMovies.slice(0, 3)); // Afficher les 3 premiers éléments transformés
...
export async function fetchSeries(page = 1, genreId = null) {
  try {
    ...
    const data = await response.json();
    console.log('Données brutes des séries (fetchSeries) - Premiers éléments :', data.results.slice(0, 3));
    ...
    console.log('Séries transformées (fetchSeries) - Premiers éléments :', transformedSeries.slice(0, 3));
...
export async function searchMoviesAndSeries(query) {
  try {
    ...
    const data = await response.json();
    console.log('Données brutes de la recherche - Premiers éléments :', data.results.slice(0, 3));
    return data.results
