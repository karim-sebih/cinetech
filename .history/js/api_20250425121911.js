...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    ...
    // Trier par popularité (ou vote_average si vous avez cette donnée)
    boxOfficeItems.sort((a, b) => (b.popularity || 0) - (a.popularity || 0));

    // Fonction pour afficher les éléments dans le Swiper
    function displaySwiperItems(wrapper, items, count) {
      if (!wrapper) {
      ...
      limitedItems.forEach(item => {
        ...
        const title = (item.type === 'movie' ? item.title : item.name) || 'Titre non disponible'; // Vérification supplémentaire
        const cardClass = item.type === 'movie' ? 'carte-film' : 'carte-serie';
        slide.innerHTML = `
          <div class="${cardClass}">
            <h3>${title}</h3>
            <img src="${imageSrc}" alt="${title}" loading="lazy">
            <button class="details-btn" data-id="${item.id}" data-type="${item.type}">Voir Détails</button>
          </div>
        `;
    ...
    displaySwiperItems(boxOfficeWrapper, boxOfficeItems, 8); // Box Office (films + séries)
