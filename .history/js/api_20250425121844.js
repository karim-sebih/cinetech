...
export async function fetchMovies(page = 1, genreId = null) {
  try {
    ...
    return data.results.map(item => ({
      id: item.id,
      title: item.title || item.original_title || 'Titre non disponible', // Utiliser original_title comme secours
...
export async function fetchSeries(page = 1, genreId = null) {
  try {
    ...
    return data.results.map(item => ({
      id: item.id,
      name: item.name || item.original_name || 'Nom non disponible', // Utiliser original_name comme secours
      poster_path: item.poster_path ? `${IMAGE_BASE_URL}w200${item.poster_path}` : 'default.jpg',
      overview: item.overview,
      popularity: item.popularity,
      vote_average: item.vote_average
