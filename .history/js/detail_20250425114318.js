...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    const endpoint = type === 'movie' 
      ? `https://api.themoviedb.org/3/movie/${id}?append_to_response=credits&language=en-FR`
...
async function fetchAndDisplayReviews(id, type) {
  try {
    ...
    const reviewsEndpoint = type === 'movie'
      ? `https://api.themoviedb.org/3/movie/${id}/reviews?language=fr-FR&page=1`
      : `https://api.themoviedb.org/3/tv/${id}/reviews?language=fr-FR&page=1`;
